...
const commit = "b32c1c81bb2444f50509ed54388758ecfab1f1c0";
const pin = `git+https://github.com/hraness/support-foundation.git#${commit}`;
...
const notice = await readFile(resolve(root, "skills/direct/THIRD_PARTY_NOTICES.md"), "utf8");
...
const result = await Bun.build({ entrypoints: [resolve(root, "scripts/support-helper-entry.ts")], target: "node", format: "esm" });
...
const destination = resolve(root, "skills/direct/scripts/support.mjs");

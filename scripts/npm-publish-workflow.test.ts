...
`;
  ...
  test("rechecks the immutable release tag at the terminal publishing boundary", async () => {
    ...
    const tarball = join(directory, "hraness-direct-0.7.13.tgz");
    ...
    try {
      ...
      const baseEnvironment = Object.freeze({
        ...
        EXPECTED_VERSION: "0.7.13",
        GITHUB_REF: "refs/tags/v0.7.13",
        GITHUB_REPOSITORY: "hraness/direct",
        GITHUB_SHA: sourceSha,
        METADATA: metadata,
        PATH: `${binaryDirectory}:${process.env.PATH ?? ""}`,
        PUBLISHED_VERSIONS_JSON: '["0.7.4","0.7.5","0.7.6","0.7.7","0.7.8","0.7.9","0.7.10","0.7.11","0.7.12"]',
      ...
      expect(`${moved.stdout}${moved.stderr}`).toContain(
        "Tag v0.7.13 changed after artifact verification",
      );
      ...
      expect(`${detached.stdout}${detached.stderr}`).toContain(
        "Tag v0.7.13 is no longer reachable from main",
      );
      expect(await Bun.file(publishMarker).exists()).toBe(false);
      ...
      expect(`${superseded.stdout}${superseded.stderr}`).toContain(
        "Tag v0.7.13 is not the newest stable tag v0.7.14",
      );
      ...
      const staleVersion = await runWorkflowScript(script, {
        ...baseEnvironment,
        PUBLISHED_VERSIONS_JSON: '["0.7.4","0.7.14"]',
      });
      expect(staleVersion.exitCode).not.toBe(0);
      expect(`${staleVersion.stdout}${staleVersion.stderr}`).toContain(
        "@hraness/direct@0.7.13 is not newer than published stable 0.7.14",
      );
      expect(await Bun.file(publishMarker).exists()).toBe(false);

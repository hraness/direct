...
export const repository = "hraness/direct";
export const repositoryId = 1306913032;
export const packageName = "@hraness/direct";
...
export function required<T>(value: T | undefined): T {
  if (value === undefined) throw new Error("Required release value is missing.");
  return value;
}
...
export function compareVersions(left: string, right: string): number {
  ...
  for (let index = 0; index < 3; index += 1) {
    if (a[index] !== b[index]) return required(a[index]) > required(b[index]) ? 1 : -1;
  }
  return 0;
}
...
export function parseManifest(value: unknown): ReleaseManifest {
  ...
  if (item.schema !== "hraness-github-release-v1" || item.repository !== repository || item.repositoryId !== repositoryId
    ...
    || archive.name !== `hraness-direct-${version}.tgz` || positive(archive.bytes, "Archive size") > 260_000
...
export async function verifyHandoff(directory: string, signed: boolean): Promise<Handoff> {
  ...
  const archive = required(files.get(manifest.archive.name));
  ...
  const pack: unknown = JSON.parse(required(files.get("npm-pack.json")).toString("utf8"));
  ...
  if (required(files.get("SHA256SUMS")).toString("utf8") !== checksums(subjects)) throw new Error("Release checksums differ from the exact handoff.");
  return { manifest, files };
}
export function admitExpectedHandoff(handoff: Handoff, expected: ReleaseEnvironment): void {
  ...
  for (const [name, variable] of [[m.archive.name, "EXPECTED_ARCHIVE_SHA256"], ["npm-pack.json", "EXPECTED_PACK_SHA256"], ["release-manifest.json", "EXPECTED_MANIFEST_SHA256"], ["SHA256SUMS", "EXPECTED_SUMS_SHA256"]] as const) {
    if (!expected[variable] || hash(required(handoff.files.get(name))) !== expected[variable]) throw new Error(`Release handoff differs from trusted ${variable}.`);
  }
}
export function admitAttempt(value: unknown, expected: { sourceSha: string; tag: string; runId: number; runAttempt: number }, completed: boolean | "canonical" = false): void {
...
export function admitRelease(value: unknown, manifest: ReleaseManifest, files: ReadonlyMap<string, Buffer>, allowMissing: boolean, expectedId?: number): { id: number; draft: boolean; present: Set<string> } {
  ...
  for (const value of release.assets) {
    ...
    // Draft browser URLs may use GitHub's temporary untagged segment.
    // Admission downloads by exact API asset ID; published URLs must be stable.
    if (!release.draft && asset.browser_download_url !== `https://github.com/${repository}/releases/download/${manifest.tag}/${asset.name}`) throw new Error("Published asset URL differs from the canonical tag and name.");
...
export function admitRemoteAssetBytes(value: unknown, files: ReadonlyMap<string, Buffer>, downloaded: ReadonlyMap<number, Buffer>): void {
  ...
  for (const value of release.assets) {
    ...
    if (seen.has(id) || typeof asset.name !== "string" || !files.has(asset.name)
      || !downloaded.get(id)?.equals(required(files.get(asset.name)))) throw new Error("Remote asset bytes differ from the exact attested handoff.");
    seen.add(id);
  }
}
...
async function requireMonotonic(version: string): Promise<void> {
  let foundTag = false;
  let completeTags = false;
  for (let page = 1; page <= 10; page += 1) {
    const values = await request(`/repos/${repository}/tags?per_page=100&page=${page}`);
    if (!Array.isArray(values) || values.length > 100) throw new Error("Version-tag inventory is invalid.");
    for (const value of values) {
      const tag = record(value, "Version tag").name;
      if (typeof tag !== "string" || !/^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$/u.test(tag)) continue;
      if (tag === `v${version}`) foundTag = true;
      if (compareVersions(version, tag.slice(1)) < 0) throw new Error("A newer stable version tag exists.");
    }
    if (values.length < 100) { completeTags = true; break; }
  }
  if (!completeTags || !foundTag) throw new Error("Newest stable-tag proof is incomplete.");
  for (let page = 1; page <= 10; page += 1) {
...
export const canonicalJobs = ["Authorize release request", "Verify", "Attest canonical artifact", "Publish"] as const;
export function admitCanonicalJobs(values: unknown, manifest: ReleaseManifest): void {
  if (!Array.isArray(values) || values.length > 1000) throw new Error("Canonical job inventory is invalid.");
  const ids = new Set<number>();
  const found = new Set<string>();
  for (const value of values) {
    const job = record(value, "Canonical job");
    const id = positive(job.id, "Canonical job ID");
    if (ids.has(id) || job.run_id !== manifest.runId || job.run_attempt !== manifest.runAttempt || job.head_sha !== manifest.sourceSha) throw new Error("Canonical job identity differs from the exact source run.");
    ids.add(id);
    if (!canonicalJobs.includes(job.name as typeof canonicalJobs[number])) continue;
    if (found.has(String(job.name)) || job.status !== "completed" || job.conclusion !== "success") throw new Error("Required canonical job has not uniquely succeeded.");
    found.add(String(job.name));
  }
  if (found.size !== canonicalJobs.length) throw new Error("Canonical publication jobs are incomplete.");
}
async function verifyCanonicalRun(manifest: ReleaseManifest): Promise<void> {
  admitAttempt(await request(`/repos/${repository}/actions/runs/${manifest.runId}/attempts/${manifest.runAttempt}`), manifest, "canonical");
  const jobs: unknown[] = [];
  for (let page = 1; page <= 10; page += 1) {
    const result = record(await request(`/repos/${repository}/actions/runs/${manifest.runId}/attempts/${manifest.runAttempt}/jobs?per_page=100&page=${page}`), "Canonical jobs");
    if (!Array.isArray(result.jobs) || result.jobs.length > 100) throw new Error("Canonical job page is invalid.");
    jobs.push(...result.jobs);
    if (result.jobs.length < 100) { admitCanonicalJobs(jobs, manifest); return; }
  }
  throw new Error("Canonical job enumeration exceeds its bound.");
}
export function admitMirrorAuthority(manifest: ReleaseManifest, expectedWorkflow: string, environment: ReleaseEnvironment,
  refValue: unknown, branchValue: unknown, comparisonValue: unknown, runValue: unknown): void {
  ...
  const run = record(runValue, "Mirror run");
  const dispatch = environment.GITHUB_EVENT_NAME === "workflow_dispatch";
  const expectedSource = dispatch ? expectedWorkflow : manifest.sourceSha;
  const expectedRef = dispatch ? "refs/heads/main" : `refs/tags/${manifest.tag}`;
  if (!sha.test(expectedWorkflow) || main.type !== "commit" || main.sha !== expectedWorkflow
    ...
    || environment.GITHUB_SHA !== expectedSource || environment.GITHUB_REF !== expectedRef
...
export async function verifyMirror(directory: string, version: string, expectedWorkflow: string): Promise<Handoff> {
  ...
  admitMirrorAuthority(manifest, expectedWorkflow, process.env,
    ...
    await request(`/repos/${repository}/actions/runs/${process.env.GITHUB_RUN_ID}/attempts/${process.env.GITHUB_RUN_ATTEMPT}`));
  // The current standalone admission helper and its calling workflows must
  // remain the same during this mirror; the artifact's source stays immutable.
  const dispatchSource = process.env.GITHUB_SHA ?? "";
...
export async function downloadMirror(directory: string, version: string, expectedWorkflow: string): Promise<Handoff> {
  ...
  const download = spawnSync("gh", ["release", "download", `v${version}`, "--repo", repository, "--dir", directory,
    "--pattern", `hraness-direct-${version}.tgz`, "--pattern", "npm-pack.json", "--pattern", "release-manifest.json",
  ...
  if (download.error || download.status !== 0) throw new Error("Canonical mirror download failed; preserve the bounded attempt evidence.");

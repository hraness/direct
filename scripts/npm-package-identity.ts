...
function expectedFilename(name: string, version: string): string {
  if (name !== "@hraness/direct") {
    throw new Error(`Expected package name must be @hraness/direct, received ${name}`);
  }
  if (!stableVersionPattern.test(version)) {
    throw new Error(`Expected package version is not stable semantic version: ${version}`);
  }
  return `hraness-direct-${version}.tgz`;
}
...
async function verifyPackArtifact(
  ...
  if (
    ...
    || !/^hraness-direct-(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.tgz$/u.test(identity.filename)
  ) {

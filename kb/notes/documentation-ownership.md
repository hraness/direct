...
Repository knowledge stays useful when each kind of truth has one primary home. The closest inherited `AGENTS.md` owns mandatory edit-time rules. `docs/` owns current architecture, adoption, verification, and wire-format procedures. Types, schemas, tests, and deterministic checkers own executable contracts. The KB owns pull-based rationale, evidence, maintained synthesis, relationships, and plans. `README.md` remains the human front door.

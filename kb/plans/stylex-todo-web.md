...
3. [x] Convert the two presentations, retain only reviewed resets, and add static recipe/production-boundary regression tests.
...
- 2026-09-09: [Complete CI 34375189988](https://github.com/hraness/direct/actions/runs/34375189988) passes at `e2339568e7f9c29332e615a574006b49e32979f0`. The tested integration commit has the same tree, `3cf1903a5f86a5cbf921ef2d8c2fd368fe76ec7c`, and includes current main `ba671caad2fcf0d3fe44ad7264fa20f9d9ab139c`. It passes thirteen Effect tests, 39 release tests with 1,122 assertions, 439 source tests with 72,425 assertions, 32 Todo tests with 135 assertions, both compiled Todo boundaries, 29 React Native tests with 261 assertions, and all three native-platform bundle boundaries. Package checks and CodeQL also pass. This is the prior checkpoint's integration evidence, not acceptance for subsequent verifier changes or native appearance.
- 2026-09-09: The independently reviewed native comparison is joined. It requires exact prebuilt baseline/current generations, source and browser identities, finite scenario/storage/activation states, responsive breakpoint edges, keyboard actions, and named geometry and computed styles. Review added descendant text and opacity sampling, real scenario-link navigation, complete bounded Git output, and preservation of primary plus cleanup failures. Scoped lint, root and example types, and ten pure tests with 154 assertions pass. Candidate-only controls must detect stylesheet removal, focus suppression and descendant-only paint changes. Transient busy-state paint is not yet observed; quiescence checks cannot close that requirement. Native execution and the separately rebuilt edit/restart/manual-refresh canary remain open.
- 2026-09-10: The final compiler join adopts immutable UI v0.5.12 and its `propertyValidationMode: "throw"` contract, fingerprint `9ac2c8448ec8f198047e824ce27a97657e05025918c01c204aa0399f94641049`. UI runtime, foundation, stylesheet and rule bytes are unchanged from v0.5.9, but compiler and package-manifest identities change. The lock update changes only the root UI specifier and its immutable package row; a frozen script-disabled install replaces one package. Independent review, root and example types, scoped lint, and 19 focused tests with 418 assertions pass, including emitted recipe checks and rejection of historical v0.5.9 busy receipts. The 82-sample native comparison at `adf8c623f81b414acda0c37f7fd301ef9bc7765d` remains evidence for v0.5.9 only. Its queued canary/busy follow-up was canceled before admission without starting a browser; all prior evidence is retained. Fresh final source validation, changed-generation native comparison, busy paint and edit/rebuild canary acceptance remain required for v0.5.12. The original baseline is unchanged.

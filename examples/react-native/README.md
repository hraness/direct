...
The example defines its worlds and claims at the package root, creates one harness session through `@cclrte/direct/testing`, and installs that session's bridge and fetch boundary atomically through `@cclrte/direct/web`. React bindings already work with React Native, while the workbench, scenarios, product ports, and platform metadata belong to the app.

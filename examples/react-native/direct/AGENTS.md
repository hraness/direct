...
- `session.ts` – activation, store, world-free manifest, cancellation, probe counters, and cleanup composition.
...
- Browser checks must sample the exact v2 schema, manifest, and probe together and bind the requested source, scenario, route, and activation hash before using fixture evidence.

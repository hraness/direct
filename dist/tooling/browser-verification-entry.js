...
function createAgentBrowser(options) {
  ...
  async function run(arguments_) {
    ...
    if (exitCode !== 0) {
      throw new Error(`agent-browser ${renderAgentBrowserCommand(commandArguments)} exited with ${exitCode}: ${tail2(stderr.trim() || stdout.trim())}`);
    }
...
async function runVerificationCommand(options) {
  ...
  if (!completed) {
    const output2 = tail2(await stopVerificationServerWithOutput(command));
    const message = `${options.label} exceeded its ${options.timeoutMs}ms deadline`;
    throw new Error(output2 === "" ? message : `${message}:
${output2}`);
  }
  const exitCode = command.exitCode();
  const output = tail2(await stopVerificationServerWithOutput(command));
  if (exitCode !== 0) {

...
- The README section between the `article:direct-a-harness-for-your-frontend` markers is also the page at hraness.com/direct. Write it for a reader who has not seen this repository: call the reader's application “your app”, not “the product”, and define “port” before using it.
- “Direct” is the product. The coverage mode `direct` means a claim that needs the real system. Always set the mode in code formatting and gloss it at its first use on a page, for example “a `direct` claim, which needs the real system”.
- Keep every reader-facing install example in `README.md` and `docs/` on one released version. `scripts/npm-publish-workflow.test.ts` checks this; when a release goes live, update those examples in one change.
